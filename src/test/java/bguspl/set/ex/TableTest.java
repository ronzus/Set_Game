...
class TableTest {
 ...
 // given tests
    ...
    //checks that the card removed from the slot successfully.
    @Test
    void removeCard_SlotRemovedFilled() throws InterruptedException {
        table.placeCard(8, 2);
        table.removeCard(2);
        assertEquals(null, table.cardToSlot[8]);
        assertEquals(null, table.slotToCard[2]);
    }

    //ensures that if the slot given points to null value it returns exception.
    @Test
    void removeCard_SlotRemovedNotFilled() throws InterruptedException {
        try {
            table.removeCard(2);
        }
        catch(NullPointerException e){
            System.out.println("NullPointerException has been thrown as expected");
        }
